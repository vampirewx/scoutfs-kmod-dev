 ...
 * radix blocks.  If the operation failed then we drop the blocks we
 * dirtied during this change and restore their refs.  We always drop
 * synthetic blocks, and we might restore their refs if they're still
 * intact -- dirtying could have overwritten the synth refs with dirty
 * refs.  Blocks are added to the head of the blocks list as they're
 * first used so we're undoing refs changes in reverse order.
 */
			    ...
			    struct radix_change *chg, int err)
{
	...
	list_for_each_entry(priv, &chg->blocks, entry) {
		...
		if (is_synth(bl->blkno) || (err < 0 && was_dirtied(priv))) {
			if (priv->ref->blkno == priv->orig_ref.blkno) {
