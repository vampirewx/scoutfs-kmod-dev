	...
	while (level--) {
		...
		if (ref->blkno) {
			...
			if (!IS_ERR_OR_NULL(bl) &&
				...
				bl = ERR_PTR(-EIO);
				scoutfs_inc_counter(sb, srch_inconsistent_eio);
			}
			if (IS_ERR(bl)) {
				ret = PTR_ERR(bl);
				goto out;
 ...
 * first search log files and then each greater size tier.
 */
...
/*
 * Perform a depth-first walk of the file's parent blocks, freeing
 * all the blocks that were allocated to the file.
 */
		     ...
		     struct scoutfs_srch_file *sfl)
{
	...
	while (level < height) {
		if (bls[level] == NULL) {
			bls[level] = scoutfs_block_read(sb,
						le64_to_cpu(ref->blkno));
			if (IS_ERR(bls[level])) {
				ret = PTR_ERR(bls[level]);
				goto out;
			}
		}
		srp = bls[level]->data;
		...
		for (i = 0; i < SCOUTFS_SRCH_PARENT_REFS; i++) {
			...
			if (ret < 0)
				goto out;
			memset(&srp->refs[i], 0, sizeof(srp->refs[i]));
		}
...
free_root:
	...
	if (ret < 0)
		goto out;
	memset(&sfl->ref, 0, sizeof(sfl->ref));
...
static void scoutfs_srch_compact_worker(struct work_struct *work)
{
	...
	/* XXX partial writes to reduce dirty metadata?  one giant commit? */
...
commit:
	...
	scres.id = scin.id;
	scres.flags = ret ? SCOUTFS_SRCH_COMPACT_FLAG_ERROR : 0;
